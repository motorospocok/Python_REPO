...
# It is required to have MTS_matrix_V131.py together
# Version 1.21 - 2021 Feb23 First version provided to public
# Version 1.22 - 2021 Marc3 Fixed save menu issues
# Version 1.3 - 2021 October 26 Introduced status check
# Version 1.31 - some defs are optimized..or at least I tried to optimize them
# Version 1.31 - enhanced socket supervision functions
# Version 1.32 - 2021 Dec 16 Intoduced thread for better handling
...
def Step_Matrix_MAX(matrix_NUM):
    obj_convert = str("w.Display") + str(matrix_NUM) #you can call it w.display, z.display etc...
    object1 = eval(obj_convert) #needs to convert the string to real object name
    object1.delete(0,2)
...
def Step_Matrix_Down(matrix_NUM):
    ...
    m1 = message1.hex()
    m2 = '02' + m1 + '03'
    t2 = threading.Thread(target=send_tcp_packet,args=(m2,))
    t2.start()
...
def send_tcp_packet(packet_to_be_send):
    ...
    try:
        ...
        w.Entry1.configure(background="purple")
        w.Entry1.insert(0, "ViZiBAGOLYHUHU!")

    t1 = threading.Thread(target=check_matrix_status)
    try:
        t1.start()
        w.Entry1.delete(0, 15)
...
def check_matrix_status():
    ...
    val = "STR" #this message must be sent to the Matrix to get status of the LEDs
    message1 = val.encode('utf-8')
    m1 = message1.hex()
    m2 = '02' + m1 + '03'
    ...
    try:
        ...
        s.connect((TCP_IP, TCP_PORT))
        s.send(MESSAGE)
        data = s.recv(1500) #receives answer from the matrix here, binary type
        s.close()
        y = str(data).replace('\\x02', '')
        y = y.replace('REM\\x03', '')
        z = re.findall(r'\d+', y) #Get the digits only form the received message
        number = 0
        number2 = 1
        while number < 24:
            if z[number + 1] == '1': # 1 means green led in the given position
                color = '#00ff00'
            if z[number + 1] == '2': # 2 means led is blank, no light
                color = 'white'
            if z[number + 1] == '0': # 0 means led is red, overvoltage
                color = '#EF347C'
            obj_convert = str("w.Display") + str(number2)
            object1 = eval(obj_convert)
            object1.configure(foreground=color) #set color according to the received data
            number2 = number2 + 1
            number = number + 2
    except:
        w.Entry1.delete(0, 15)
    ...
    val = "ST" #this command request the attenuation set of the Matrix
    message1 = val.encode('utf-8')
    m1 = message1.hex()
    m2 = '02' + m1 + '03'
    ...
    try:
        s.connect((TCP_IP, TCP_PORT))
        s.send(MESSAGE)
        data = s.recv(1500) #receives answer from the matrix here
        s.close()
        y = str(data).replace('\\x02', '')
        y = y.replace('REM\\x03', '')
        z = re.findall(r'\d+', y)
        ...
        number2 = 1
        while number < 24:
            obj_convert = str("w.Display") + str(z[number])
            object1 = eval(obj_convert)
            object1.delete(0,2) # delete all digits from the given display
            object1.insert(0,str(z[number+1])) #update given display with the actual received value
            number=number+2
    except:
        w.Entry1.delete(0, 15)
        w.Entry1.configure(background="purple")
...
# Common definiton for group increasing and decreasing
def group_button_click(element,current_value,prm,operation1):
    if operation1 == '-' :
        calc_value = current_value - 1
    else:
        calc_value = current_value + 1
    if calc_value > 95:
        element.delete(0, 2)
        element.insert(0, str("95"))
        calc_value = 95
    if calc_value < 0:
        element.delete(0, 2)
        element.insert(0, str("0"))
        calc_value = 0
    if calc_value > 0 or calc_value == 0:
        element.delete(0, 2)
        element.insert(0, str(calc_value))
    result = 'A' + str(prm) + 'P' + str(calc_value)
    return(result)
...
def GR_Dw_Left_Click(p1):
    ...
    if che1.get() == 1:
       ...
       object1 = eval(obj_convert)
       current_att = eval(object1.get())
       val1 = group_button_click(object1, current_att, temp1,oper)
       val = val+val1
...
def GR_Dw_Right_click(p1):
    val = ""
    if che1.get() == 1:
       w.Display1.delete(0,2) #I did not see reason to write a new definition
...
def About():
    ...
    popup = tk.Tk()
    popup.wm_title("!")
    label = ttk.Label(popup, text="Version 1.32 - 2021 December 16", font="TkFixedFont")
...
if __name__ == '__main__':
    import MTS_matrix_V132
    MTS_matrix_V132.vp_start_gui()
